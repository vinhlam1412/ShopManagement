...
using ShopManagement.Blazor.Client;
using ShopManagement.Blazor.Components;
using Volo.Abp;
...
[DependsOn(
    ...
    typeof(ShopManagementHttpApiModule),
...
public class ShopManagementBlazorModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        ...
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                typeof(ShopManagementResource),
                typeof(ShopManagementDomainModule).Assembly,
                typeof(ShopManagementDomainSharedModule).Assembly,
                typeof(ShopManagementApplicationModule).Assembly,
                typeof(ShopManagementApplicationContractsModule).Assembly,
                typeof(ShopManagementBlazorModule).Assembly
            );
        });
        ...
        PreConfigure<OpenIddictBuilder>(builder =>
        {
            ...
            {
                options.AddAudiences("ShopManagement");
        ...
        Configure<AbpBundlingOptions>(options =>
        {
            ...
            options.StyleBundles.Configure(
                ...
                bundle =>
                {
                    ...
                    bundle.AddFiles(new BundleFile("/ShopManagement.Blazor.styles.css", true));
                }
            );
        });
    }
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<ShopManagementDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}ShopManagement.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<ShopManagementDomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}ShopManagement.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<ShopManagementApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}ShopManagement.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<ShopManagementApplicationModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}ShopManagement.Application"));
                options.FileSets.ReplaceEmbeddedByPhysical<ShopManagementBlazorModule>(hostingEnvironment.ContentRootPath);
            });
        }
    }
    ...
    private void ConfigureSwaggerServices(IServiceCollection services)
    {
        services.AddAbpSwaggerGen(
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "ShopManagement API", Version = "v1" });
    ...
    private void ConfigureRouter(ServiceConfigurationContext context)
    {
        ...
        {
            options.AppAssembly = typeof(ShopManagementBlazorModule).Assembly;
    ...
    private void ConfigureAutoApiControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(ShopManagementApplicationModule).Assembly);
        });
    }
    ...
    private void ConfigureAutoMapper()
    {
        ...
        {
            options.AddMaps<ShopManagementBlazorModule>();
        });
    }
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "ShopManagement API");
        });
        app.UseAuditing();
